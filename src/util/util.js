...
export const toObj = (str) => {
  if (typeof str == 'string') {
    try {
      ...
      console.log('error：请输入json格式的字符串');
      return str;
    }
  }
}
...
// 生成从1~n的随机抽取的length个数组
export const RandomArray = (n, maxNum) => {
  var numArr = [];
...
// 获取当前时间前n分钟的时间
export const getMin = (n, time = new Date()) => {
  time = new Date(time);
  time = time.getTime();
  return moment(time + n * 1000 * 60).format("YYYY-MM-DD HH:mm:ss")
}
