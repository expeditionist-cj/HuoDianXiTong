...
export default {
  ...
  methods: {
    ...
    onExport() {
      ...
      excel("/datamonitor/hdtxindicatoryear/export", {
        ...this.query,
        plantName: plant,
      }).then((res) => {
        let data = res.data;
        this.area_plant = area ? (plant ? area + "/" + plant : area) : "";
        let excelName = `${this.year}年 ${this.area_plant} 脱硝计划指标.xlsx`;
    ...
    changePlantPower(data) {
      this.rowData["xsj"] = Number(data * this.rowData["xsj_rate"]);
    ...
    changeXsj_rate(data) {
      // 【还原剂耗用量】=【计划发电量】*【还原剂耗用率】；
      this.rowData["xsj"] = Number(data * this.rowData["plan_power"]);
    },
    // 改变装置耗电率
    change_hd_rate(data) {
      this.rowData["hd"] = Number(
