<template>
  <div class="zcrud" v-loading="loading">
    ...
    >
      ...
      <!-- 操作栏 -->
      <el-table-column
        v-if="txconsume_update || txconsume_del"
        fixed="right"
        align="center"
        label="操作"
        width="100"
      >
        <template slot-scope="scope">
          ...
          <div v-else>
            ...
            >
            <el-button
              v-if="txconsume_del && scope.row.day == '调整'"
              @click="doDelete(scope.row)"
              type="text"
              size="small"
              >删除</el-button
...
export default {
  ...
  created() {
    this.txconsume_update = this.permissions["txconsume_update"]; // 修改
    this.txconsume_del = this.permissions["txconsume_del"]; // 删除
  },
  mounted() {},
  watch: {},
  methods: {
        ...
        update_nx_ems({
          ...
          if (msg == "success") {
            ...
            this.timer = setTimeout(() => {
              this.$store.commit("SET_ISNXSUCCESS", true);
    ...
    handleClick(row) {
      ...
      this.options = days.map((item) => {
        return {
          label:
            item == "调整" ? "调整" : this.year + "-" + this.month + "-" + item,
          value: item == "调整" ? 32 : item,
        };
      });
      this.date = day == "调整" ? 32 : day;
